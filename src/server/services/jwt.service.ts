...
export class JwtService {
    ...
    public static async generateToken(userId: string): Promise<string> {
        ...
        const token = await jwt.sign(payload, JwtService.rsaPrivateKey, { algorithm: 'RS256', expiresIn: '1h' });
        return token;
    }
    ...
    private static async readFile(filePath: string): Promise<string> {
        return new Promise<string>((resolve, reject) => {
            fs.readFile(filePath, (err, data) => {
                if (err != null) { reject(err); } else { resolve(data.toString()); }
            });
        });
    }
    private static async loadKeyPair(): Promise<void> {
        JwtService.rsaPrivateKey = await JwtService.readFile(JwtService.rsaPrivateKeyFile);
        JwtService.rsaPublicKey = await JwtService.readFile(JwtService.rsaPublicKeyFile);
    }
    private static async generateKeyPairIfNotExist(): Promise<void> {
        const privateKeyFileExist = await JwtService.fileExists(JwtService.rsaPrivateKeyFile);
        const publicKeyFileExist = await JwtService.fileExists(JwtService.rsaPublicKeyFile);
