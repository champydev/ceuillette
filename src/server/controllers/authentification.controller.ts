...
export class AuthentificationController
{
    ...
    async login(req: restify.Request, res: restify.Response, next: restify.Next) {
        if (req.body == null) return next(new Error('Missing body'));
        if (req.body.email == null) return next(new Error('Missing email'));
        if (req.body.hash == null) return next(new Error('Missing hash'));
       
        const dataService = new DataService();
        const sql = 'select * from public.user where email=$1 and hash=$2';
        console.log("login "+req.body.email+" "+req.body.hash );
        const result = await DataService.executeQuery(sql, [req.body.email, req.body.hash]);
    ...
    async refresh(req: restify.Request, res: restify.Response, next: restify.Next) {
        let token = req.header('Authorization');
        if (token == null){
